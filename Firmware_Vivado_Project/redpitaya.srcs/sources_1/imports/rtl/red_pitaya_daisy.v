...
(
   // SATA connector
   output wire     [  2-1: 0] daisy_p_o       ,  //!< TX data and clock [1]-clock, [0]-data
   output wire     [  2-1: 0] daisy_n_o       ,  //!< TX data and clock [1]-clock, [0]-data
   input wire      [  2-1: 0] daisy_p_i       ,  //!< RX data and clock [1]-clock, [0]-data
   input wire      [  2-1: 0] daisy_n_i       ,  //!< RX data and clock [1]-clock, [0]-data

   // Parallel data
   input wire                 ser_clk_i       ,  //!< high speed serial clock, used for TX
   input wire                 dly_clk_i       ,  //!< delay clock, used for IDELAY
   // TX port
   input wire                 par_clk_i       ,  //!< parallel TX clock - data clock, must be in ration with ser_clk_i
   input wire                 par_rstn_i      ,  //!< parallel TX reset - active low
   output wire                par_rdy_o       ,  //!< parallel TX data - ready to receive new
   input wire                 par_dv_i        ,  //!< parallel TX data valid
   input wire      [ 32-1: 0] par_dat_i       ,  //!< parallel TX data to send
   // RX port
   output wire                par_clk_o       ,  //!< parallel RX clock   !!! not in relation with TX par_clk_i !!!
   output wire                par_rstn_o      ,  //!< parallel RX reset - active low
   output wire                par_dv_o        ,  //!< parallel RX data valid
   output wire     [ 32-1: 0] par_dat_o       ,  //!< parallel RX data received

   output wire     [  8-1: 0] debug_o         ,  //!< some debug

   // System bus
   input wire                 sys_clk_i       ,  //!< bus clock
   input wire                 sys_rstn_i      ,  //!< bus reset - active low
   input wire      [ 32-1: 0] sys_addr_i      ,  //!< bus address
   input wire      [ 32-1: 0] sys_wdata_i     ,  //!< bus write data
   input wire      [  4-1: 0] sys_sel_i       ,  //!< bus write byte select
   input wire                 sys_wen_i       ,  //!< bus write enable
   input wire                 sys_ren_i       ,  //!< bus read enable
...
/*OBUFDS #(.IOSTANDARD ("DIFF_SSTL18_I"), .SLEW ("SLOW")) i_OBUF_clk
(
  .O  ( daisy_p_o[1]  ),
  .OB ( daisy_n_o[1]  ),
  .I  ( txs_clk       )
...
OBUFDS #(.IOSTANDARD ("DIFF_SSTL18_I"), .SLEW ("SLOW")) i_OBUF_dat
(
  .O  ( daisy_p_o[0]  ),
  .OB ( daisy_n_o[0]  ),
  .I  ( txs_dat       )
...
//Commenting this module for now to avoid converting it all from 16 bit to 32 bit
...
reg            rxp_dv_n     ;
reg  [32-1: 0] rxp_dat_n    ; 
reg  [ 4-1: 0] tx_rx_new    ;
reg            tx_rx_dv     ;
reg  [32-1: 0] tx_rx_dat    ; 
...
always @(*) begin
   case (tx_cfg_sel)
      3'h0 : begin txp_dat <= 32'h0         ;   txp_dv <= 1'b0         ; end
      ...
      3'h3 : begin txp_dat <= 32'h2BDA60E4      ;   txp_dv <= txp_rdy   ; end  // training data
...
always @(posedge sys_clk_i) begin
   ...
   casez (sys_addr_i[19:0])
     ...
     20'h0000C : begin sys_ack_o <= sys_ack;       sys_rdata_o <= { rxp_dat }           		                             ; end
