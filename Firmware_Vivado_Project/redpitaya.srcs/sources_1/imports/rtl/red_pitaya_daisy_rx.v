 ...
 * (for 8-bits). In our case parallel data width is 16-bits so we need to slip
 * also bytes (par_cnt) to achive correct received data. To have this
 * functionality working correctly module needs known received value. For now
 * this value is 32'h2BDA60E4.
 *
...
(
   ...
   output reg [ 32-1: 0] par_dat_o          //!< parallel data
);
...
BUFR #(.SIM_DEVICE("7SERIES"), .BUFR_DIVIDE("4")) i_BUFR_clk
(
  .O   (  par_clk      ),
  .CE  (  1'b1         ),
  .CLR ( !cfg_en_i     ),
...
always @(posedge par_clk or negedge cfg_en_i) begin
   if (cfg_en_i == 1'b0) begin
      ...
      par_rstn_r <= {par_rstn_r[32-2:0], dly_rdy} ;
      par_rstn   <=  par_rstn_r[32-1];
   end
end
...
reg  [ 3-1: 0] byteslip_cnt  ;
wire [ 8-1: 0] rxp_dat      ;
reg  [ 8-1: 0] rxp_dat_1r   ;
reg  [ 8-1: 0] rxp_dat_2r   ;
reg  [ 8-1: 0] rxp_dat_3r   ;
reg            par_dv       ;
reg  [32-1: 0] par_dat      ;
reg            par_ok       ;
reg  [ 2-1: 0] par_cnt      ;
...
// connection training - bitslip and byte "shift"
always @(posedge par_clk) begin
   if (par_rstn == 1'b0) begin
      bitslip     <=  1'b0 ;
      bitslip_cnt <=  5'b0 ;
      ...
      bitslip <= (bitslip_cnt[3:2] == 3'b10) && (par_dat != 32'h2BDA60E4) && par_dv && !par_ok ;


      // shifting input data by 8bits every 8 bitslips
      if (par_train && bitslip && !par_ok)
         byteslip_cnt <= byteslip_cnt + 3'h1 ;
      else if (!par_train || par_ok)
         byteslip_cnt <= 3'h0 ; 

      if ((byteslip_cnt == 3'h7) && bitslip)
         par_cnt <= par_cnt ; // "shift" input data by 8
      else
      ...
      if (par_train && (par_dat == 32'h2BDA60E4) && par_dv)
         par_ok <= 1'b1 ;
      else if ((bitslip_cnt[3:2] == 3'b10) && (par_dat != 32'h2BDA60E4) && par_dv)
         par_ok <= 1'b0 ;
...
// combining data - concentrate 4 bytes
always @(posedge par_clk) begin
   if (par_rstn == 1'b0) begin
      rxp_dat_1r <=  8'h0 ;
      rxp_dat_2r <=  8'h0 ;
